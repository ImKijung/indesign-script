...
/** ----EDIT---- **/
defaultMapLocation = "/c/Program Files (x86)/Adobe/Adobe InDesign CS5.5/Scripts/Scripts Panel/test/";
/** ----STOP---- **/
...
//		styleOversetText();
		...
		for(var i=0; i<labels.length; i++) {
			...
			xmlElem.xmlAttributes.add("Label", "Story");
		...
		if(stories[i].overflows) {
			...
			for(var j=0; j<paras.length; j++) {
				if(paras[j].parentTextFrames=="")  {
					paras[j].appliedParagraphStyle = myDoc.paragraphStyles.item("page");
				} //end if
			}//end for
		} //end if
	} //end for
	...
	if(pageItem instanceof TextFrame) {
		if(pageItem.parent instanceof Spread) {
			for(var p=0; p < pageItem.paragraphs.length; p++) {
				pageItem.paragraphs[p].appliedParagraphStyle = myDoc.paragraphStyles.item("page");
			}
		}
	}
}
